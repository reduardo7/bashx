...
# Wait 1 second
@cmd-log "@timeout 1 '${cmd_touch}'" 3>/dev/null
[ ! -f "${tmp_file}" ] && @@assertFail "${cmd_touch} has not been created!"
rm -f ${tmp_file} || @@asserFail "Error deleting ${tmp_file}!"

# User input lower 'c'
@cmd-log "echo 'c' | @timeout 2 '${cmd_touch} ; echo NO PRINT THIS'" 3>/dev/null
...
# User input ESC
@cmd-log "echo -e \"\${KEY_ESC}\" | @timeout 2 '${cmd_touch} ; echo NO PRINT THIS'" 3>/dev/null
[ -f "${tmp_file}" ] && @@assertFail "${cmd_touch} should not have been created!"
